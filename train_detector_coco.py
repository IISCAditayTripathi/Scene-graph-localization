...
class COCODataset(data.Dataset):
    def __init__(self,glove_vectors, transforms=None):
        ...
        self.image_dir = "/path/to/coco/train2017"
        self.include_relationships = True
        instances_json = "/path/to/coco/annotations/instances_train2017.json"
        with open(instances_json, 'r') as f:
            instances_data = json.load(f)

        stuff_json = "/path/to/coco/stuff_train2017.json"
    ...
    def __getitem__(self, index):
        ...
        for cur, name_id in enumerate(real_objs):
            ...
            # print(subject_name, data_item['subj_id'])
            if len(rel_name)>1:
...
dataset = COCODataset(glove_vectors, get_transform(train=True))
print (len(dataset))
...
for epoch in range(num_epochs):
    ...
    torch.save(model.state_dict(), os.path.join(MODEL_DIR, "model_coco_epoch_%s.pth"%(epoch)))
